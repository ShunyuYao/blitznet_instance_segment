...
class Detector(object):
    def __init__(self, sess, net, loader, config, no_gt=False, folder=None, profiler=None):
        self.sess = sess
        self.net = net
        self.loader = loader
        self.config = config
    ...
    def draw(self, img, dets, cats, scores, name, gt_bboxes, gt_cats):
        ...
        # image.save('./Results/test/{}.png'.format(name))
        # print('img save')
    ...
    def feed_forward(self, img, seg_gt, w, h, name, gt_bboxes, gt_cats, draw=False, step=0):
        ...
        if args.use_profile:
            outputs = self.sess.run(net_out,
                                    feed_dict=feed_dict,
                                    options=self.run_options,
                                    run_metadata=self.run_metadata)

            if step % args.profile_step == 0:
