...
def extract_batch(dataset, config):
    with tf.device("/cpu:0"):
        ...
        bbox_matches = tf.reshape(bbox_matches, (args.instance_num, 4))
        ...
        gt_matches = tf.reshape(gt_matches, (args.instance_num,))
        ...
        return tf.train.shuffle_batch([im, inds, refine, cats, ins_cats, seg, ins, match_roi_info, bbox_matches, gt_matches],
...
def train(dataset, net, config):
    image_ph, inds_ph, refine_ph, classes_ph, ins_classes, seg_gt, ins, match_roi_info, bbox_match, gt_match = extract_batch(dataset, config)
    # ins_summary = ins
    ...
    tf.summary.tensor_summary('tensor/bbox_matches', bbox_match)
    tf.summary.tensor_summary('tensor/gt_matches', gt_match)
