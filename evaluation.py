...
class COCOEval(Evaluation):
    ...
    def process_image(self, img_id, number, step=0):
        img = self.loader.load_image(img_id)
        # Image.fromarray(img.astype(np.uint8)).save('./Results/test/{}.png'.format(img_id))
        # print('img save')
        ...
        out = self.detector.feed_forward(img, seg_gt, w, h, img_id,
                                         gt_bboxes, gt_cats, True, step)
        segment = out[3]
        segment0 = segment[segment != 0]
        print(segment0[segment0 != 15])
        detections = []
        if args.detect:
    ...
    def evaluate_network(self, ckpt):
        ...
        for i in bar(range(len(filenames))):
            img_id = filenames[i]
            detections.extend(self.process_image(img_id, i, step=i))
